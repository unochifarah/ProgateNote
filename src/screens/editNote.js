...
const EditNote = ({ setCurrentPage, editNote, currentNote, labelColors }) => {
  const [title, setTitle] = useState(currentNote?.title || '');
  const [desc, setDesc] = useState(currentNote?.desc || '');
  const [selectedLabels, setSelectedLabels] = useState(currentNote?.labels || []);

  useEffect(() => {
    if (currentNote) {
      setTitle(currentNote.title);
      setDesc(currentNote.desc);
      setSelectedLabels(currentNote.labels);
    }
  }, [currentNote]);
  ...
  const handleSubmit = () => {
    editNote(currentNote.id, title, desc, selectedLabels);
    setCurrentPage('home');
  };
